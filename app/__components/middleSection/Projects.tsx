...
const Projects = () => {
  ...
  return (
    <div id="projects" className="w-full flex justify-center">
      <div className="w-[84%] flex flex-col items-center mt-[10%]">
        <span className="font-bold text-[2rem] tracking-wider text-primary mb-12">Projects</span>
        <div className="flex flex-col gap-32">
          {projects.map((project, index) => (
            ...
            >
              <div className="md:w-[70%] mb-6 md:mb-0 relative z-0 grayscale hover:grayscale-0 hover:scale-105 transition duration-300 ease-in-out">
                <a
                  href={project.link}
                  target="_blank"
              ...
              <div className={`md:w-[50%] flex flex-col ${index % 2 !== 0 ? 'md:items-start md:ml-8' : 'md:items-end md:mr-8'} z-10`}>
                <span className="font-bold text-[1.7rem] text-text mb-4">{project.title}</span>
                <div className={`bg-secondary p-6 text-text ${index % 2 !== 0 ?"text-left":"text-right"}`}>
                  <span className={`font-normal text-[1.2rem] leading-relaxed ${index % 2 !== 0 ?"mr-12":"ml-12"}`}>
                    {project.description}
                  </span>
                </div>
                <ul className="list-none flex gap-3 flex-wrap mt-4 text-[1.2rem]">
